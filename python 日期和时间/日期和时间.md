...
- 将获取的时间戳传递给`localtime`函数
- 直接调用`localtime`函数

## 获取格式化时间

 将获取的当前时间传递给`asctime`函数
...
获取**2018年1月**日历`calender.month(2018, 1)`
...
| 3    | [time.clock()](http://www.runoob.com/python3/python3-att-time-clock.html) 用以浮点数计算的秒数返回当前的CPU时间。用来衡量不同程序的耗时，比time.time()更有用。 | [实例](http://www.runoob.com/python3/python3-att-time-clock.html) |
...
| 7    | [time.mktime(tupletime)](http://www.runoob.com/python3/python3-att-time-mktime.html) 接受时间元组并返回时间辍（1970纪元后经过的浮点秒数）。 | [实例](http://www.runoob.com/python3/python3-att-time-mktime.html) |
